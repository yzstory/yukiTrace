...
## 当前阶段
阶段 26 已完成：第四款紫色手写 Trace Logo 与全部当前改动已提交、push 并部署到 HTTPS 生产环境。小程序独立平台发布仍需微信开发者工具与凭据。

### 阶段 26：手写 Logo 与整包发布
- [x] 保存选定原图、生成页面字标与各尺寸图标，更新缓存版本
- [x] 验证全部当前代码、API 与移动端流程
- [x] 生产数据库及旧镜像备份、部署迁移、HTTPS 验证
- [x] 全部提交并推送 origin
- **状态：** complete
...
### 阶段 14：离线记录队列
- [x] IndexedDB 队列（idb）：文字操作 + 照片 Blob 两个 store，变更事件广播
- [x] `/api/sync` 回放接口：复用与在线完全相同的 Server Action，校验/汇率/时区/算路行为一致
- [x] `useOfflineForm` 包装花费、宝宝状态、条目表单；离线时按钮文案变「离线记一笔」
- [x] 照片离线存 Blob，联网后自动上传
- [x] SyncBadge 顶部浮条：离线提示、待同步数量、手动重试、同步完成提示，联网事件自动回放并 router.refresh
- [x] 失败重试上限 5 次，错误写回队列条目，避免坏数据卡死
- [x] 浏览器验证：断网 → 记账 → 入队 → 恢复网络 → 自动同步 → 数据落库渲染
- **状态：** complete
...
### 阶段 17：AI 第二档（跨旅程记忆）
- [x] 向量索引 `Embedding` 模型：站点/条目/花费/日记/照片说明/旅程共 6 类文本片段；**不引入 pgvector**（postgres:16-alpine 无该扩展），家庭级数据量在应用层算余弦相似度足够，省一次数据库镜像迁移风险
- [x] 增量重建：只对文本变化的片段重新求向量，删除的记录自动清理索引；写入后经 `after()` 后台触发
- [x] 全局问答 `/ask` + `/api/ai/ask`：5 个只读工具（searchMemories / listTrips / compareSpending / findPlacesVisited / onThisDay），支持语音提问
- [x] 未配置向量模型时自动降级为关键词匹配，功能不消失
- [x] 那年今日：旅程列表顶部横向卡片，显示几年前的今天、地点与宝宝当时月龄
- [x] 消费洞察：与上一段旅程的日均对比、最大分类占比、宝宝相关占比、疑似重复记账、单笔异常；**纯统计不调模型**，稳定可解释
- [x] 智能导入：粘贴航班邮件/酒店订单/行程单 → AI 解析成条目草稿 → 用户勾选后落库（不直接写库）
- [x] 验证：mock 网关跑通导入解析、索引构建与增量（第二次 0 条）、语义搜索返回排序结果；那年今日与洞察正确渲染
- **状态：** complete

### 阶段 18：AI 第三档（生成与开放）
- [x] 年度回顾 `/year/[year]`：跨旅程统计（旅程数/天数/城市/里程/航班/花费/宝宝成长）+ AI 写「给宝宝的信」，可保存长图；`/me` 列出所有可回顾年份
- [x] 家庭日记合成 `generateFamilyDigest`：按成员归集当天的记录，合成一段共同日记（多人旅程才显示入口）
- [x] PDF 相册 `/trips/[id]/album`：打印样式排版 + 浏览器「存储为 PDF」，不引入 PDF 库因而中文不会缺字；照片按 AI 评分挑选、排除票据
- [x] MCP Server `/api/mcp`：JSON-RPC（initialize / tools/list / tools/call / ping），复用跨旅程只读工具；独立 MCP 令牌鉴权（只存哈希，明文只展示一次）；`/settings/mcp` 管理令牌并给出配置片段
- [x] 回放视频：**未做**。地图回放已有，录制成 MP4 需要 canvas 录屏 + 编码，投入产出比低于其他项，用「保存长图 + 分享链接」替代
...
### 阶段 19：视觉重设计（暖色旅行手帐）
- [x] 设计系统：暖纸底 / 赭红 / 鼠尾草 / 琥珀 token，深色暖炭；Fraunces 展示衬线自托管
- [x] 首页 4:5 照片卡、那年今日琥珀故事条、悬浮胶囊底栏、旅程头图与展示数字、吸顶日头、账本英雄数字、登录与侧栏品牌
- [x] 全站截图审查两轮，修正字体配置与页面标题字体规则
- [x] 单测 47 / 浏览器 8 条通过，已部署
- **状态：** complete


### 阶段 19：旅行护照（可玩性）
- **目标：** 每到一个新城市盖一枚章，章上文字由 AI 写；有收集感、有仪式感、可保存分享
- **数据：** 新表 `CityStamp(tripId, city, line, source, inkedAt)`，唯一 (tripId, city)；章本身由站点城市实时推导，只有 AI 文案落库
- **页面：** `/passport` 护照本：头部统计（城市 / 旅程 / 里程 / 飞行、绕地球百分比），章按首次到访排序；未盖的是虚线轮廓，点一下盖章（动效），「全部盖章」一键；保存为图片
- **入口：** 「我」页顶部护照卡（N 城）；旅程页发现新城市未盖章时显示提示条
- **AI：** 文案 ≤ 24 字，纯文本，基于城市、日期、月龄、站点、第一次；未配置 AI 用模板兜底
- **验证：** 单测（推导、模板、色相）+ WebKit e2e（注册 → 种子 2 段旅程 3 城 → 盖章 → 全部盖章）+ 截图审查 → `deploy.sh --with-migrate`
- [x] Schema + migration
- [x] lib/passport.ts 推导与文案
- [x] 页面、章组件、动效、保存图片
- [x] 入口（我 / 旅程页提示）
- [x] 测试、截图、README（单测 55 / e2e 通过；截图审查两轮，修正弧线文字方向与统计换行）
