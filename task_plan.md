...
### 阶段 2：核心记录功能（第一版可用）
- [ ] 旅程 CRUD：封面、标题、日期范围、同行人、宝宝出生日期（自动计算月龄）
- [ ] 首页旅程卡片流（封面 + 城市 + 天数 + 总花费 + 脚印数）
- [ ] 站点 Stop：高德 POI 搜索 / 地图选点录入坐标，到达/离开时间，类型图标
- [ ] 站点间距离：Haversine 直线距离 + 高德路径规划（驾车/步行）实际距离与时长，结果缓存到 DB
- [ ] 条目 Entry：航班（航班号/起降/座位）、租车（车行/车型/取还车点/里程）、火车/打车、住宿、餐食、游玩、购物、「此刻」
- [ ] 花费 Expense：金额、货币、分类、可挂任何条目或独立；汇率表 + 自动折算 CNY；标记「宝宝相关」
- [ ] 照片上传：前端直传 OSS（STS 临时凭证 + 签名），sharp 生成缩略图或用 OSS 图片处理参数；备注 Markdown
- [ ] 旅程时间线视图：按天分组，纵向时间轴，站点卡片 + 站间距离小灰字
- [ ] 快速记录浮钮 `+`：一排大图标，3 步内完成一条记录
- **状态：** pending
