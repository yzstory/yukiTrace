...
test("整理中心：核对重复、编辑 AI 记录、撤销与家庭历史", async ({ page }) => {
  ...
  try {
    ...
    await page.goto(`/trips/${tripId}/organize`);
    ...
    await page.getByRole("link", { name: "家庭操作历史" }).click();
    await expect(page.getByRole("heading", { name: "家庭操作历史" })).toBeVisible();
    await expect(page.getByText("整理测试妈妈 · 修改 花费", { exact: true })).toBeVisible();
    await expect(page.getByText("名称：午餐测试 → 晚餐已纠正", { exact: true })).toBeVisible();
    await expect(page.getByText("整理测试妈妈 · 撤销 花费", { exact: true })).toBeVisible();
  } finally {
