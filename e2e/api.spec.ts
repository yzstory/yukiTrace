...
test("JSON API：令牌登录与旅程读写主线", async ({ playwright, baseURL }) => {
  ...
  try {
    ...
    expect(record.values.title).toBe("地铁");
    const patched = await authed.patch(`/api/v1/trips/${tripId}/records/expense/${expenseId}`, { data: { version: record.version, values: { ...record.values, title: "地铁一日券", amount: "830" } } });
