...
- AI 助手：接入任意 OpenAI 兼容接口，用自然语言查询或写入旅程数据，支持拍照或选择本地照片识别票据、游记与装备清单生成。
...
- 旅程整理：在旅程「更多 → 旅程整理」查看本设备待同步内容、疑似重复账单、缺少关联地点、AI 待核对记录与识别失败照片。
- 家庭操作历史：在「更多 → 家庭操作历史」查看核心记录的操作者、AI/手动来源及修改前后内容，按页浏览；旧数据不补造历史。
...
- 历史覆盖核心地点、条目、花费、照片、宝宝状态和日记的已接入写入路径，不是全站安全审计；分享访问和旧版数据没有追溯记录。
