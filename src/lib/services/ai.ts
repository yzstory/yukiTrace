...
import { badRequest, tooMany, unavailable } from "@/lib/api/errors";
...
function requireAi() {
  if (!aiConfigured()) throw unavailable("AI 未配置");
}
function gate(key: string) {
  const g = rateLimit(key, LIMITS.aiGenerate.limit, LIMITS.aiGenerate.windowMs);
  if (!g.ok) throw tooMany(`生成太频繁，请 ${g.retryAfterS} 秒后再试`);
}
...
export async function generateDailyDraft(actor: Actor, tripId: string, date: string, tone: "default" | "to_baby" = "default") {
  await assertTripAccess(actor.userId, tripId, "EDITOR");
  requireAi();
  gate(`ai:draft:${tripId}`);
  const audited = auditedDb({ tripId, userId: actor.userId, source: "ai" });
  ...
  const facts = [
    ...
    .filter(Boolean)
    .join("\n");
  if (!facts.includes("站点") && !facts.includes("条目") && !facts.includes("花费")) throw badRequest("这一天还没有记录，先记点什么吧");
  ...
  await audited.dailyNote.upsert({ where: { tripId_date: { tripId, date: day } }, update: { aiDraft: draft }, create: { tripId, date: day, content: "", aiDraft: draft } });
 ...
 */
export async function generateFamilyDigest(actor: Actor, tripId: string, date: string) {
  await assertTripAccess(actor.userId, tripId, "EDITOR");
  requireAi();
  gate(`ai:digest:${tripId}`);
  const audited = auditedDb({ tripId, userId: actor.userId, source: "ai" });
  const trip = await db.trip.findUniqueOrThrow({ where: { id: tripId } });
  const day = dayOf(date);
  ...
  if (stops.length === 0 && expenses.length === 0 && photos.length === 0) throw badRequest("这一天还没有记录");
...
/** 生成整段旅程的游记与总结要点 */
export async function generateTripSummary(actor: Actor, tripId: string) {
  await assertTripAccess(actor.userId, tripId);
  requireAi();
  gate(`ai:summary:${tripId}`);
...
export async function generatePackingList(actor: Actor, tripId: string) {
  await assertTripAccess(actor.userId, tripId, "EDITOR");
  requireAi();
  gate(`ai:packing:${tripId}`);
  ...
  try {
    ...
    throw unavailable("清单解析失败，请重试");
  }
  if (items.length === 0) {
    log.warn("ai.packingList empty", { tripId });
    throw unavailable("没有生成内容");
  }
