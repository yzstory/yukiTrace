...
export async function POST(req: NextRequest) {
  ...
  const limited = rateLimit(`ai:stt:${userId}`, LIMITS.aiChat.limit, LIMITS.aiChat.windowMs);
  ...
  const done = log.timer("ai.transcribe", { userId: userId, bytes: audio.size });
  const r = await transcribeAudio(audio, String(form.get("language") ?? "zh"));
  if ("error" in r) {
    log.warn("ai.transcribe failed", { userId: userId, reason: r.error });
