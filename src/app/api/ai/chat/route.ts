...
export async function POST(req: NextRequest) {
  ...
  const done = log.timer("ai.chat", { userId: session.userId, tripId, messages: messages.length, attachments: attachments.length });
  const result = streamText({
    ...
    tools: tripTools({ tripId, userId: session.userId, homeCurrency: trip.homeCurrency, now, canEdit, timezone: trip.timezone, startDate: trip.startDate, endDate: trip.endDate, attachments }),
